...
```python
from scng.discovery.ssh import SSHClient, SSHClientConfig
...
```python
from scng.discovery.ssh import TextFSMParser
...
```python
from scng.discovery.ssh import SSHCollector
...
python -m scng.discovery.ssh templates

# Test against device
python -m scng.discovery.ssh test 192.168.1.1 -u admin -p secret -v

# With legacy mode
python -m scng.discovery.ssh test 192.168.1.1 -u admin --legacy

# Save results to JSON
python -m scng.discovery.ssh test 192.168.1.1 -u admin -o results.json

# Parse output from file
python -m scng.discovery.ssh parse output.txt cisco_ios_show_cdp
```
...
- Returns `scng.discovery.models` objects for consistency with SNMP
