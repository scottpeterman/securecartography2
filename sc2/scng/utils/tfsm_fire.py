...
class ThreadSafeConnection:
    ...
    def get_connection(self):
        ...
        if not hasattr(self._local, 'connection'):
            # Read-only: the template DB ships inside site-packages, which may
            # be read-only, and the engine only ever reads it. Opening read-only
            # also keeps SQLite from creating journal files next to it.
